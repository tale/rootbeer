use crate::{LockedInstall, LockedPackage, LockedSource};
use rootbeer_catalog::{is_sha256, validate_https};
...
impl crate::PublishedArtifact {
    pub fn validate(
        ...
        crate::catalog::validate_apps(&package.provides.apps)?;
        if recipe
            .bins
            .paths()
        ...
        for path in package.provides.bins.values() {
            crate::realize::validate_relative_path("published command", path)
        ...
        match &package.install {
            ...
            LockedInstall::Archive { strip_prefix, .. } => {
                if let Some(path) = strip_prefix {
                    crate::realize::validate_relative_path("published archive prefix", path)
                        .map_err(|e| e.to_string())?;
                }
            }
            LockedInstall::Binary { path } => {
                crate::realize::validate_relative_path("published binary", path)
                    .map_err(|e| e.to_string())?;
            }
            LockedInstall::Directory { .. } => {
                return Err("published artifacts cannot install local directories".into())
            }
        }
        let LockedSource::Url { url, sha256 } = &package.source else {
            return Err(format!(
                "{key}: published artifacts must use HTTPS or GHCR URLs"
            ));
        };
        if url.starts_with("ghcr://") {
            let blob = crate::ghcr::GhcrBlob::parse(url)?;
