...
impl OfficialIndexSource {
    ...
    pub fn verify(&self, manifest: &Manifest) -> Result<(), String> {
...
mod tests {
    ...
    fn fixture(_root: &Path) -> (OfficialIndexSource, Ed25519KeyPair, Vec<u8>) {
        ...
        (
            source,
            key,
            serde_json::to_vec(&crate::artifact::fixture()).unwrap(),
        )
    }
