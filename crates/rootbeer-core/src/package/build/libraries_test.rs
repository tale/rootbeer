...
fn static_library_chain_builds_and_runs_after_dependencies_are_removed() {
    ...
    for (name, dependency) in [
        ...
        let install = if is_library {
            ...
            "mkdir -p \"$1/bin\"; cp consumer \"$1/bin/\"".into()
        };
    ...
    let artifact =
        build_package_in_cache(&catalog, "consumer", &output, 2, &inputs, &downloads).unwrap();
...
fn compact_library_recipes_require_checks_and_new_index_schema() {
    ...
    assert_eq!(super::super::ArtifactIndex::schema_for(&catalog), 4);
    ...
    let index = super::super::ArtifactIndex {
        schema: 4,
        catalog_sha256: catalog.sha256(),
        catalog,
        artifacts: BTreeMap::new(),
    };
    let mut decoded: super::super::ArtifactIndex =
